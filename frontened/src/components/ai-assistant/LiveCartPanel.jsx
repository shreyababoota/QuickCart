...
export default function LiveCartPanel({
  ...
  // IDs already in cart — used for ✓ / disabled state on add buttons
  ...
  const handleAddAlternative = (alt, originalProductId, fallbackCategory) => {
    onAddToCart({
      product_id: alt.product_id,
      name:       alt.name,
      price:      alt.price,
      category:   alt.category || fallbackCategory || '',
      brand:      alt.brand,
    });
        ...
        {cart.length === 0 ? (
          ...
          <div>
            ...
            <ul className="divide-y divide-gray-50 px-1 mt-1">
              {cart.map((item) => {
                const sugg = (inlineSuggestions || {})[item.id];
                    ...
                    {sugg && sugg.suggestions?.length > 0 && (
                      ...
                      >
                        ...
                        >
                          ...
                          <button
                            onClick={() => onDismissInline(item.id)}
                          ...
                          {sugg.suggestions.slice(0, 3).map((s) => {
                            ...
                            return (
                              <div key={s.product_id} className="flex items-center gap-2 px-2.5 py-2">
                                ...
                                {/* Add button */}
                                <button
                                  onClick={() => !inCart && onAddToCart({ product_id: s.product_id, name: s.name, price: s.price, category: s.category || '', brand: s.brand })}
                                  disabled={inCart}
                          ...
                          {rec.alternatives.slice(0, 3).map((alt) => {
                            ...
                            return (
                              <div key={alt.product_id} className="flex items-center gap-2">
                                ...
                                <button
                                  onClick={() => handleAddAlternative(alt, rec.original.product_id, rec.original.category)}
                                  disabled={inCart}
                                  className={`flex-shrink-0 text-xs font-semibold px-2 py-1 rounded-lg transition-colors ${
                                    inCart
                                      ? 'bg-gray-100 text-gray-400 cursor-default'
